...
function FreezableBadge({ style }) {
	return (
		<View style={[{ backgroundColor: "#d4eef5", borderRadius: 999, paddingHorizontal: 8, paddingVertical: 3, flexDirection: "row", alignItems: "center", gap: 4 }, style]}>
			...
			<Text style={{ fontSize: 10, fontWeight: "700", color: "#2a5f8f" }}>Freezable</Text>
		</View>
	);
}
...
function FeaturedCard({ recipe, isPro, onPress }) {
	...
	return (
		<TouchableOpacity onPress={onPress} activeOpacity={0.88} style={{ width: 200, borderRadius: 18, overflow: "hidden", backgroundColor: C.white, shadowColor: "#000", shadowOpacity: 0.1, shadowRadius: 8, elevation: 3 }}>
			{recipe.imageUrl ? (
			...
			<View style={{ position: "absolute", top: 8, left: 8, flexDirection: "row", gap: 5 }}>
				<View style={{ backgroundColor: "rgba(212,160,23,0.92)", borderRadius: 999, paddingHorizontal: 7, paddingVertical: 3 }}>
			...
			<View style={{ padding: 12 }}>
				<Text style={{ fontWeight: "700", fontSize: 13, color: C.primaryPinkDark, marginBottom: 6 }} numberOfLines={2}>{recipe.title}</Text>
				<View style={{ flexDirection: "row", gap: 6, flexWrap: "wrap" }}>
					<View style={{ backgroundColor: C.bgGreen,   borderRadius: 999, paddingHorizontal: 7, paddingVertical: 2 }}><Text style={{ fontSize: 10, fontWeight: "700", color: "#2e7d52" }}>{recipe.ageGroup}</Text></View>
					<View style={{ backgroundColor: C.bgPurple, borderRadius: 999, paddingHorizontal: 7, paddingVertical: 2 }}><Text style={{ fontSize: 10, fontWeight: "700", color: C.primaryPurple }}>{recipe.time}</Text></View>
...
export function RecipesScreen({ isPro, recipes, favouriteRecipeIds, onUpgradePro, onToggleFav, onLogRecipe, user, jumpToRecipeId = null, onJumpHandled }) {
	const { C } = useTheme();
	const s = useStyles();
	...
	return (
		<ScrollView ref={scrollViewRef} showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16, paddingBottom: 24 }}>
			...
			{/* Search */}
			<View style={{ flexDirection: "row", alignItems: "center", backgroundColor: C.white, borderRadius: 14, borderWidth: 1.5, borderColor: searchQuery ? C.primaryPurple : C.borderLight, paddingHorizontal: 14, paddingVertical: 10, gap: 10, shadowColor: "#000", shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 }}>
				<Icon name="search" size={16} color={searchQuery ? C.primaryPurple : C.mutedText} />
				<TextInput value={searchQuery} onChangeText={setSearchQuery} placeholder="Search by ingredient or recipe name…" placeholderTextColor={C.mutedText} style={{ flex: 1, fontSize: 14, color: C.textCharcoal, fontWeight: "500", paddingVertical: 0 }} autoCorrect={false} autoCapitalize="none" returnKeyType="search" />
			...
			<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingHorizontal: 2 }}>
				{ageGroups.map((ag) => (
					<TouchableOpacity key={ag} onPress={() => setFilterAge(ag)} style={{ backgroundColor: filterAge === ag ? C.primaryPurple : C.white, borderRadius: 999, paddingHorizontal: 16, paddingVertical: 8, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 4, elevation: 1 }}>
						<Text style={{ fontSize: 13, fontWeight: "700", color: filterAge === ag ? C.white : C.mutedText }}>{ag === "all" ? "All Ages" : ag}</Text>
					</TouchableOpacity>
				))}
