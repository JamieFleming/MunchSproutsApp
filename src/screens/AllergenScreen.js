...
const HOW_TO_STEPS = [
	{
		...
		desc: 'When you give your baby a food with an allergen, tap "Add Food" and select the allergen from the list, and record as "Neutral". It will appear as "In Progress".',
	},
	{
		step: "2",
		icon: "clock",
			...
			<View
				...
				<View style={{ flex: 1 }}>
					...
					<Text style={{ fontSize: 11, color: "#d4860a", marginTop: 2 }}>
						Introduced {a.daysSinceFirstIntro ?? a.daysSinceFirst} day
						{(a.daysSinceFirstIntro ?? a.daysSinceFirst) !== 1 ? "s" : ""} ago
						· Any delayed reaction in the 48hr window?
					</Text>
				</View>
			</View>
			<View style={{ flexDirection: "row", gap: 8 }}>
				<TouchableOpacity
					onPress={() => onAllergenCheckIn?.(a.value, "safe")}
					activeOpacity={0.8}
					style={{
						flex: 1,
						flexDirection: "row",
				...
				<TouchableOpacity
					onPress={() => onAllergenCheckIn?.(a.value, "allergic")}
					activeOpacity={0.8}
					style={{
						flex: 1,
						flexDirection: "row",
...
function AllergenRow({ al, index, total, onViewInLog, onAddWithPrefill }) {
	const { C } = useTheme();
	const cfg = STATUS_CFG[al.status];
	const isNotTried = al.status === "Not Tried";
	const canTap = !isNotTried && onViewInLog;

	const handlePress = () => {
		if (isNotTried && onAddWithPrefill) {
			Alert.alert(
				`Introduce ${al.value}?`,
				`${al.value} hasn't been tried yet. Would you like to log a new food with this allergen?`,
				[
					{ text: "Not now", style: "cancel" },
					{ text: "Log it now", onPress: () => onAddWithPrefill(al.value) },
				],
			);
		} else if (canTap) {
			onViewInLog(al.value);
		}
	};

	return (
		<TouchableOpacity
			activeOpacity={canTap || isNotTried ? 0.7 : 1}
			onPress={handlePress}
			style={{
				flexDirection: "row",
				alignItems: "center",
	...
	return (
		<ScrollView
			...
			{checkIns.length > 0 && (
				<View
					...
					<Text
						...
						It's been 48+ hours since introducing these allergens. Did your
						baby show any delayed reaction in the window after eating?
					</Text>
					{checkIns.map((a) => (
						<CheckInCard
							key={a.value}
							allergen={a}
							onAllergenCheckIn={onAllergenCheckIn}
						/>
					))}
				</View>
			)}
			...
			<Text style={[s.sectionTitle, { marginBottom: 10 }]}>
				How to Log an Allergen
			</Text>
			<View
				style={[s.card, { marginBottom: 18, padding: 0, overflow: "hidden" }]}>
				{HOW_TO_STEPS.map((item, i) => (
					<StepRow
						key={item.step}
						item={item}
						isLast={i === HOW_TO_STEPS.length - 1}
					/>
				))}
				<TouchableOpacity
			...
			<View
				...
				{allergenStatus.map((al, i) => (
					<AllergenRow
						...
						al={al}
						index={i}
						total={allergenStatus.length}
						onViewInLog={onViewInLog}
						onAddWithPrefill={onAddWithPrefill}
					/>
				))}
			</View>
